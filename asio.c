...
#define MAX_ENVIRONMENT_SIZE        32
...
typedef struct IWineASIOImpl
{
    ...
    char                        jack_connect_name[ASIO_MAX_NAME_LENGTH];
...
HIDDEN ASIOBool STDMETHODCALLTYPE Init(LPWINEASIO iface, void *sysRef)
{
    ...
    if (This->jack_connect_name[0])
        This->jack_input_ports = jack_get_ports(This->jack_client, This->jack_connect_name, NULL, JackPortIsOutput);
    else
    ...
    if (This->jack_connect_name[0])
        This->jack_output_ports = jack_get_ports(This->jack_client, This->jack_connect_name, NULL, JackPortIsInput);
    else
...
static BOOL configure_driver(IWineASIOImpl *This)
{
    ...
    *application_name = 0;
    ...
    /* autoconnect to ports with the given name instead of physical ports */
    size = GetEnvironmentVariableA("WINEASIO_CONNECT_TO_NAME", environment_variable, ASIO_MAX_NAME_LENGTH);
    if (size > 0 && size < ASIO_MAX_NAME_LENGTH)
        strcpy(This->jack_connect_name, environment_variable);
